...
const int TURNOS_CAMBIO = 10; // N turnos hasta que desaparecen

class Jugador{
    private:
    int jugadori,  jugadorj;
    int pasosRestantes, pasosDados;                                             //Bateria
    ...
    void moverJugador(char direccion, string grid[][10]){
        if (pasosRestantes <= 0) return;                                // No puede moverse sin batería
        bool seMovio = false;
        switch(direccion){
            case 'L':
...
int main() {
    ...
    while (window.isOpen()) {
        while (const optional event = window.pollEvent()) {
            ...
            if (!juegoTerminado && event->is<Event::KeyPressed>()) {
        ...
        // Recolectar item
        int i = p.getPosXJugador(), j = p.getPosYJugador();
        if (grid[i][j] == "K") {
            p.recogerItem();
            grid[i][j] = ".";
        ...
        // Paredes temporales
        for (auto it = paredesTemporales.begin(); it != paredesTemporales.end(); ) {
            it->second++;
        ...
        if (!juegoTerminado) {
            if (grid[i][j] == "G") {
                juegoTerminado = true;
                mensajeFinal.setString("Llegaste a la meta!");
        ...
        for (int j = 0; j < cols; ++j) {
            ...
            if(grid[i][j] == "#"){                                      //Pared
                ...
                if (rand() % 100 < 15) {                                // 15% de paredes serán temporales
                    paredesTemporales[{i, j}] = 0;                      // empieza en turno 0
                }
